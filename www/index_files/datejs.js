    ...
    $.fn.datejs = function (options) {
        ...
        $("#calender td").die().live("click", function () {
            ...
            if (dv != "&nbsp;") {
                var str = "";
                /***
                * 2014-05-14 update gjk 
                * 修改显示日期格式为:yyyy-MM-dd (原版:yyyy-M-d) 
                ***/
                var monthFull = $("#month").val();
                if (parseInt(monthFull) < 10) { monthFull = "0" + monthFull; }
                if (parseInt(dv) < 10) { dv = "0" + dv; }
                /***
                * end
                ***/
                if (options.isTime) {
                    var nd = new Date();
                    str = $("#year").val() + options.fuhao + monthFull + options.fuhao + dv + " " + nd.getHours() + ":" + nd.getMinutes() + ":" + nd.getSeconds();
                } else {
                    str = $("#year").val() + options.fuhao + monthFull + options.fuhao + dv;
                }
